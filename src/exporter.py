...
# Para Oracle 9i, usar PACKAGE_BODY (com underscore) ao inv├®s de "PACKAGE BODY"
...
class OracleExporter:
    ...
    def copy(self, schemas: Iterable[str]) -> None:
        ...
        logger.info("=" * 70)
        logger.info("INICIANDO PROCESSO DE EXPORTA├ç├âO")
        logger.info("=" * 70)
        ...
        for idx, schema in enumerate(schema_list, 1):
            ...
            # Estabelecer conex├Áes separadamente para melhor tratamento de erros
            try:
                ...
                try:
                    ...
                    logger.info("Ô£ô Conex├úo com banco de origem estabelecida")
                except Exception as e:
                    logger.error("Ô£ù Falha ao conectar ao banco de ORIGEM (11g): %s", e)
                    logger.error("  DSN: %s", self.source.dsn)
                    logger.error("  Usu├írio: %s", self.source.user)
                ...
                try:
                    ...
                    logger.info("Ô£ô Conex├úo com banco de destino estabelecida")
                except Exception as e:
                    logger.error("Ô£ù Falha ao conectar ao banco de DESTINO (9i): %s", e)
                    logger.error("  DSN: %s", self.target.dsn)
                    logger.error("  Usu├írio: %s", self.target.user)
                    if self.target.client_path:
                        ...
                        logger.error("")
                        logger.error("  Para conectar ao Oracle 9i, voc├¬ PRECISA configurar:")
                        logger.error("  ORACLE_9I_CLIENT_PATH=C:\\caminho\\para\\instantclient_11_2")
                        logger.error("  no arquivo .env ou como vari├ível de ambiente")
                    raise
                ...
                raise
            logger.info("")
            logger.info("Ô£ô Schema %s conclu├¡do com sucesso", schema)
            logger.info("")
        
        logger.info("=" * 70)
        logger.info("EXPORTA├ç├âO CONCLU├ìDA COM SUCESSO")
    ...
    def _copy_ddl(self, source_conn: cx_Oracle.Connection, target_conn: cx_Oracle.Connection, schema: str) -> None:
        ...
        # Usar execute ao inv├®s de callproc para melhor compatibilidade com cx_Oracle 6.x
        try:
            cursor.execute("""
                BEGIN
                ...
                END;
            """)
            logger.info("Ô£ô Configura├º├úo do DBMS_METADATA conclu├¡da")
        except cx_Oracle.DatabaseError as e:
            error, = e.args
            # Se DBMS_METADATA n├úo estiver dispon├¡vel, continuar sem as transforma├º├Áes
            if error.code in (4043, 6550):  # Package n├úo existe ou n├úo acess├¡vel
                logger.warning("DBMS_METADATA n├úo dispon├¡vel, continuando sem transforma├º├Áes")
            else:
                raise
        ...
        for object_type in DDL_OBJECTS:
            ...
            if object_type == "PACKAGE_BODY":
                # No all_objects, o tipo ├® "PACKAGE BODY" (com espa├ºo)
            ...
            try:
                ...
                if error.code == 31600:  # ORA-31600: invalid input value for parameter OBJECT_TYPE
                    logger.warning("    Tipo de objeto '%s' n├úo suportado pelo DBMS_METADATA neste banco, pulando...", object_type)
                    objects = []
                else:
                    raise
            if objects:
                ...
                for name, ddl in objects:
                    try:
                        ...
                        if hasattr(ddl, "read"):
                            # Se for um LOB, ler todo o conte├║do
                            try:
                                if hasattr(ddl, "size"):
                                    # LOB tem m├®todo size(), ler tudo de uma vez
                                ...
                                logger.warning("    ÔÜá Erro ao ler LOB completo, tentando m├®todo alternativo: %s", e_read)
                                # Fallback: tentar ler como string
                                ddl_str = str(ddl)
                        else:
                            ddl_str = str(ddl)
                        
                        # Verificar se o DDL est├í vazio ou muito curto (pode indicar truncamento)
                        if not ddl_str or len(ddl_str.strip()) < 10:
                            logger.warning("    ÔÜá %s.%s: DDL muito curto ou vazio, pode estar truncado", schema, name)
                            if object_type in ("PACKAGE", "PACKAGE_BODY"):
                                logger.warning("    Tentando obter DDL completo usando m├®todo alternativo...")
                                ...
                                try:
                                    ...
                                    logger.error("    Ô£ù N├úo foi poss├¡vel obter DDL alternativo: %s", e_alt)
                        
                        # Para tabelas, fazer DROP se j├í existir antes de criar
                        if object_type == "TABLE":
                            if self._table_exists(target_conn, schema, name):
                                logger.info("    Tabela %s.%s j├í existe, removendo para recriar...", schema, name)
                        ...
                        logger.info("    Ô£ô %s.%s criado/aplicado", schema, name)
                        total_objects += 1
                    except cx_Oracle.DatabaseError as exc:
                        error, = exc.args
                        if error.code in (955, 2264, 1430):
                            if object_type == "TABLE":
                                # Se a tabela ainda existe ap├│s tentar criar, tentar dropar e recriar
                                logger.warning("    ÔÜá Tabela %s.%s j├í existe, tentando remover para recriar...", schema, name)
                                if self._drop_table(target_conn, schema, name):
                                    # Tentar criar novamente ap├│s remover
                                    try:
                                        execute_non_query(target_conn, ddl_str)
                                        logger.info("    Ô£ô %s.%s recriado com sucesso", schema, name)
                                        total_objects += 1
                                    except Exception as e3:
                                        logger.error("    Ô£ù Erro ao recriar %s.%s ap├│s DROP: %s", schema, name, e3)
                                        logger.warning("    Continuando com pr├│ximo objeto...")
                                else:
                                    logger.error("    Ô£ù N├úo foi poss├¡vel remover tabela %s.%s para recriar", schema, name)
                            else:
                                logger.warning("    ÔÜá %s.%s j├í existe, ignorando", schema, name)
                                total_objects += 1
                        elif error.code in (911, 900, 6550):  # ORA-00911, ORA-00900, ORA-06550: invalid SQL/character/end-of-file
                            logger.warning("    ÔÜá %s.%s: DDL cont├®m problemas (c├│digo %s), tentando alternativa...", schema, name, error.code)
                            ...
                            if object_type in ("PACKAGE", "PACKAGE_BODY") and error.code in (900, 6550):
                                try:
                                    ...
                                    ddl_str = self._get_package_ddl_from_source(source_conn, schema, name, object_type)
                                    execute_non_query(target_conn, ddl_str)
                                    logger.info("    Ô£ô %s.%s criado/aplicado (via ALL_SOURCE)", schema, name)
                                    total_objects += 1
                                    continue
                                except Exception as e_source:
                                    logger.warning("    Falha ao obter de ALL_SOURCE: %s", e_source)
                            
                            # Tentar executar statement por statement ap├│s limpeza
                            try:
                                ...
                                with target_conn.cursor() as cur:
                                    for stmt in statements:
                                        if stmt.strip():
                                            cur.execute(stmt)
                                target_conn.commit()
                                logger.info("    Ô£ô %s.%s criado/aplicado (ap├│s limpeza)", schema, name)
                                total_objects += 1
                            except Exception as e2:
                                logger.error("    Ô£ù Erro ao criar %s.%s ap├│s limpeza: %s", schema, name, e2)
                                # Continuar com pr├│ximo objeto ao inv├®s de falhar completamente
                                logger.warning("    Continuando com pr├│ximo objeto...")
                        else:
                            logger.error("    Ô£ù Erro ao criar %s.%s: %s (c├│digo %s)", schema, name, error.message, error.code)
                            # Continuar com pr├│ximo objeto ao inv├®s de falhar completamente
                            logger.warning("    Continuando com pr├│ximo objeto...")
            else:
                logger.info("    Nenhum objeto do tipo %s encontrado", object_type)
        
        logger.info("Ô£ô Etapa 1/2 conclu├¡da: %d objeto(s) DDL processado(s)", total_objects)
    ...
    def _copy_data(self, source_conn: cx_Oracle.Connection, target_conn: cx_Oracle.Connection, schema: str) -> None:
        ...
        for idx, table in enumerate(tables, 1):
            ...
            if not self._table_exists(target_conn, schema, table):
                logger.error("    Ô£ù Tabela %s.%s N├âO EXISTE no destino!", schema, table)
            ...
            try:
                ...
                if row_count > 0:
                    ...
                    logger.info("    Ô£ô %d linha(s) copiada(s) com sucesso", row_count)
                    total_rows_copied += row_count
                else:
                    logger.info("    Ô£ô Tabela vazia (0 linhas)")
            except Exception as e:
                logger.error("    Ô£ù Erro ao processar tabela %s.%s: %s", schema, table, e)
                # Continuar com pr├│xima tabela ao inv├®s de parar tudo
                logger.warning("    Continuando com pr├│xima tabela...")
        
        if tables_missing:
            logger.error("")
            logger.error("=" * 70)
            logger.error("ATEN├ç├âO: %d TABELA(S) N├âO FORAM CRIADAS NO DESTINO", len(tables_missing))
            logger.error("=" * 70)
        ...
        logger.info("")
        logger.info("Ô£ô Etapa 2/2 conclu├¡da: %d tabela(s) processada(s), %d linha(s) copiada(s) no total", 
    ...
    def _insert_rows(
        ...
        with connection.cursor() as cursor:
            ...
            connection.commit()
            if total_batches > 1:
                logger.info("      Ô£ô Commit realizado")
    ...
    def _drop_table(connection: cx_Oracle.Connection, schema: str, table: str) -> bool:
        ...
        Retorna True se a tabela foi removida, False se n├úo existia.
        """
        ...
        try:
            ...
            error, = exc.args
            logger.warning("    ÔÜá Erro ao remover tabela %s.%s: %s (c├│digo %s)", schema, table, error.message, error.code)
            ...
            try:
                ...
                logger.error("    Ô£ù N├úo foi poss├¡vel remover tabela %s.%s: %s", schema, table, e2)
                return False

    @staticmethod
    def _get_package_ddl_from_source(connection: cx_Oracle.Connection, schema: str, package_name: str, object_type: str) -> str:
        """
        Obt├®m o DDL de um package ou package body a partir de ALL_SOURCE.
        ├Ütil quando DBMS_METADATA retorna DDL truncado ou inv├ílido.
        """
        cursor = connection.cursor()
        try:
            ...
            if not ddl or len(ddl.strip()) < 10:
                raise ValueError(f"DDL obtido de ALL_SOURCE est├í vazio ou muito curto para {schema}.{package_name}")
    ...
    def _truncate_target_table(connection: cx_Oracle.Connection, schema: str, table: str) -> None:
        ...
        if not OracleExporter._table_exists(connection, schema, table):
            logger.error("    Ô£ù ERRO: Tabela %s.%s n├úo existe no destino! N├úo ├® poss├¡vel copiar dados.", schema, table)
            raise ValueError(f"Tabela {schema}.{table} n├úo existe no destino. Execute a c├│pia de DDL primeiro.")
        ...
        try:
            ...
            error, = exc.args
            if error.code == 942:
                logger.error("    Ô£ù Tabela %s.%s n├úo encontrada no destino", schema, table)
                raise
            else:
                raise
